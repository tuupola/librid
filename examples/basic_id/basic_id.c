...
main(void)
{
    ...
    rid_set_uas_id(&basic_id, "1ABCD2345EF678XYZ");

    hexdump(&basic_id, sizeof(basic_id));

    char uas_id[21];
    rid_get_uas_id(&basic_id, uas_id, sizeof(uas_id));
    rid_ua_type_t ua_type = rid_get_ua_type(&basic_id);
    rid_basic_id_type_t id_type = rid_get_basic_id_type(&basic_id);
